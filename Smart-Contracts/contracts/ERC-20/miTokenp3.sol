...
contract MiToken{
    ...
    constructor(uint256 initialSupply){
        owner = msg.sender; //el deployer es el owner
    ...
    function transferFrom(address sender, address recipent, uint256 amount) public returns (bool){
        require(_balances[sender] >= amount, "Saldo insuficiente");
        require(_allowances[sender][msg.sender] >= amount, "No tienes permiso para gastar tanto");
    ...
    function mint(address account, uint256 amount) public onlyOwner {
        require(account != address(0), "Cuenta invalida");
        _totalSupply += amount;
    ...
    function burn(address account, uint256 amount) public onlyOwner{
        require(account != address(0), "Cuenta invalida");
        require(_balances[account] >= amount, "Saldo insuficiente");
        _totalSupply -= amount;
        _balances[account] -= amount;
